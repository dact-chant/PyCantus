...
def build_argument_parser():
    ...
    parser.add_argument('-o', '--output_csv', action='store',
                        help='Store the source information into this CSV.')
    parser.add_argument('--source_id', action='store',
                        help='Scrape information for this source ID. (Used mainly for testing/debugging.)')

    parser.add_argument('--scrape_inventories', action='store_true',
                        help='Scrape also the CSV files of sources inventories from Cantus DB.')
    parser.add_argument('--inventories_dir', action='store',
                        help='Inventories will be scraped into this directory.')
...
def main(args):
    ...
    scraper = CantusDBScraper()

    # Your code goes here
    if args.source_id:
        source_data = scraper.scrape_source_id(args.source_id)
        print(source_data.to_dict())

        source_inventory = scraper.scrape_source_csv(args.source_id)
        print(source_inventory)

    else:
        sources = scraper.scrape_sources_listing()
        logging.info('{} source listings scraped'.format(len(sources)))

        if args.output_csv:
            with open(args.output_csv, 'w', newline='') as fh:
                csv_writer = csv.DictWriter(fh, fieldnames=SourceData.get_field_names(),
                                            delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                csv_writer.writeheader()
                for source in sources:
                    csv_writer.writerow(source.to_dict())

        if args.scrape_inventories:
            if not args.inventories_dir:
                raise ValueError('Cannot scrape inventories CSV without the target directory specified.')
            if not os.path.isdir(args.inventories_dir):
                os.mkdir(args.inventories_dir)

            for source in sources:
                source_id = source.drupal_path.split('/')[-1]
                logging.info('Scraping inventory for source ID {}'.format(source_id))

                inventory = scraper.scrape_source_csv(source_id)
                output_path = os.path.join(args.inventories_dir, source_id + '.csv')
